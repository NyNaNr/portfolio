...
この方法は、誰が選ばれるかが予測できないため、児童生徒にとってはスリリングです。また、教員にとっては、授業への参加を効果的に促進できる指名方法となります。

今回開発したアプリで、この指名方法を令和にアップデートしたいです！

# このアプリを作るきっかけ

児童生徒を指名する方法はさまざまな種類があります。そのうちの一つがランダム指名（くじ引き指名）です。それならただ単に教員がランダムに指名すればよいと考えがちですが、児童生徒の不満を生むこともあります。児童生徒からしてみれば、教員の意図が入り込む余地があるからです。教師の無意識の偏見からくる不平等を減少させ、より公平なクラスルーム環境を構築するためにも、ランダムに指名する時は教員の意図が入らないプログラムをエクセルで実装し、普段から使っていました。思わぬ副作用で児童生徒からの反応もよく、名前がランダムに表示されている時間は、誰が当たるか分からない緊張感が漂います。

嬉しいことに同僚の教員にも使ってもらえるようになりました！しかしそのエクセルファイルを紹介するたびに児童生徒の名簿を関数の範囲に指定する方法を教えなければならず、またエクセルは関数を誤って削除してしまったり、操作方法を忘れてしまうなど、利用するための少し高い壁がありました。
ちょうどそのころ、私は Python にドハマりしている時期でした。そこでプログラミングで Web アプリを作れば、どんな環境でもブラウザがあれば、誰でも全国どこでも簡単にアクセスし、かつ分かりやすく使ってもらえるのではないかと思いました。プログラミングで教員の役に立ちたいと思い、この機能を Web アプリで実装しようと作り始めました。
